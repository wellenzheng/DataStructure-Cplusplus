...
int info_fb_check = 0;               /*0������Ҫ���н��ּ��,1:��Ҫ���н��ּ��*/
...
int height2;
int diroff[9];
int firstPlayer = 1; //��¼������˭�����ڽ����жϣ���ʼ��Ϊ1������Ķ��֣�
...
static const char *get_cmd_param(const char *cmd, const char *input)
{
  ...
  n1 = strlen(cmd);
  n2 = strlen(input);
  if (n1 > n2 || _strnicmp(cmd, input, n1))
    return NULL; /* it is not cmd */
  input += strlen(cmd);
...
static void do_command()
{
  ...
  {
    if ((info = get_cmd_param("max_memory", param)) != 0)
      info_max_memory = atoi(info);
    ...
    if ((info = get_cmd_param("game_type", param)) != 0)
      info_game_type = atoi(info);
    ...
    if ((info = get_cmd_param("folder", param)) != 0)
      strncpy(dataFolder, info, sizeof(dataFolder) - 1);
    if ((info = get_cmd_param("fb_check", param)) != 0)
      info_fb_check = atoi(info);
#ifdef DEBUG_EVAL
    if ((info = get_cmd_param("evaluate", param)) != 0)
    {
      if (parse_coord(info, &x, &y))
        brain_eval(x, y);
    }
#endif
  ...
  {
    if (sscanf(param, "%d %d", &width, &height) == 1)
      height = width;
    start();
    brain_init();
  }
